...
import { Box, Button, Heading, HStack, Image, Link, Spacer, Text, Tooltip, useColorMode, VStack } from "@chakra-ui/react"
...
export default function Component() {
  ...
  {main_state.header_state.withNav.map((pvkwwjao, i) => (
  <Link as={NextLink} href={pvkwwjao.at(1)} key={i} sx={{"opacity": "0.85", "transition": "opacity 600ms ease", "_hover": {"textDecoration": "None", "opacity": "1"}}}>
  ...
  <Box sx={{"paddingLeft": ["1rem", "1rem", "1rem", "4rem", "9rem"], "transition": "all 550ms ease"}}>
  <Heading size={`2xl`} sx={{"paddingBottom": "1.5rem"}}>
  {`Error 404`}
</Heading>
  <Heading size={`md`}>
  ...
  <HStack spacing={`2rem`} sx={{"alignItems": "end", "transition": "opacity 500ms ease 500ms"}}>
  {main_state.header_state.withNav.map((qqmlspmi, i) => (
  <Link as={NextLink} href={qqmlspmi.at(1)} key={i} sx={{"opacity": "0.85", "transition": "opacity 600ms ease", "_hover": {"textDecoration": "None", "opacity": "1"}}}>
  <Heading size={`s`} sx={{"paddingTop": "0.3rem", "color": "white", "fontWeight": "semibold"}}>
  {qqmlspmi.at(0)}
</Heading>
</Link>
